...
module.exports = {
    ...
    async execute(interaction) {
        ...
        if(!config.server.roles.adm.some(roleId => interaction.member.roles.cache.has(roleId))) return interaction.reply({content: "Este comando não foi feito para que __você__ o use. Apenas administradores.", flags: [Discord.MessageFlags.Ephemeral]})
        ...
        interaction.reply({
            ...
            let acao_contexto = (await interaction.guild.channels.cache.get(config.server.channels.context).messages.fetch()).sort().map(msg => msg.content).join('\n\n');
